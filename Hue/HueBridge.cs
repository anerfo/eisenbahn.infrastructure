...
namespace Hue
{
    public class HueBridge
    {
        ...
        private HueLight Default = HueLight.Empty;
        ...
        private void ParseLights(string json)
        {
            ...
            {
                Lights.TryAdd(light.Key, HueLight.Parse(light.Key, light.Value));
            }
        }
        ...
        public void SetLight(HueLight light, HueLightState state)
        {
            if (Lights != null && IsAuthenticated)
            {
                SetLightStatus(light.key, HueLightState.JsonCommand(light.GetState(), state));
            }
        }
        
        public void SetLight(HueLight light, int? sat = null, int? bri = null, int? hue = null)
        {
            if (Lights != null && IsAuthenticated)
            {
        ...
        public void SetLightCT(HueLight light, int ct)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
        ...
        public void SetLightXY(HueLight light, int x, int y)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
                state.xy = new decimal[] { x, y };
                SetLightStatus(light.key, HueLightState.JsonCommand(light.GetState(), state));
            }
        }

        public void SetLightOnOff(HueLight light, bool on)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
        ...
        public void SetLightColormode(HueLight light, string colormode)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
        ...
        public void SetLightEffect(HueLight light, string effect)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
        ...
        public void SetLightAlert(HueLight light, string alert)
        {
            if (Lights != null && IsAuthenticated)
            {
                var state = light.GetState();
        ...
        public HueLight GetLight(string name)
        {
            if (Lights != null)
            {
                foreach (var light in Lights)
                {
                    if (light.Value.name.Equals(name, StringComparison.InvariantCultureIgnoreCase))
                    {
                        return light.Value;
                    }
                }
