...
namespace Hue.Contracts
{
    ...
    {
        private HueLightState state { get; set; }
        ...
        {
            return new HueLightState(state);
        }

        public static HueLight Parse(string key, dynamic d)
        {
        ...
        public static HueLight Empty
        {
            get
            {
                var instance = new HueLight();
                instance.key = "0";
                instance.state = new HueLightState();
                instance.type = "Dummy";
                instance.name = "Empty";
                instance.modelid = "Dummy";
                instance.swversion = "0";
                return instance;
            }
        }
    }
    ...
    public class HueLightState
    {
        ...
        public HueLightState(HueLightState state)
        {
            this.on = state.on;
            this.bri = state.bri;
            this.hue = state.hue;
        ...
        public string JsonDiff(HueLightState update)
        {
            ...
            if (ct != update.ct) result += "\"ct\": " + update.ct + ", ";
            if (colormode != update.colormode) result += "\"colormode\": " + update.colormode + ", ";
            if (xy != null && xy.Length == 2 && update.xy != null && update.xy.Length == 2)
            {
                if (xy[0] != update.xy[0] || xy[1] != update.xy[1])
                {
                    result += "\"xy\": [" + update.xy[0].ToString("0.0000") + "," + update.xy[1].ToString("0.0000") + "], ";
                }
            }
            if (effect != update.effect) result += "\"effect\": " + update.effect + ", ";
            //instance.xy = new decimal[] { d["xy"][0], d["xy"][1] };
            //instance.alert = d["alert"];
            //instance.effect = d["effect"];
            //instance.colormode = d["colormode"];
